$(document).ready(function(){
	...
	$("#botonGrupos").click(function(){
		$("#grupo").html('');
			$.getJSON("http://localhost:8888/api/grupos", function(data){
	...
	$("#botonGrupoBorrarT").click(function(){
			...
			$("#grupo").html('');
			$.ajax({
				url: 'http://localhost:8888/api/grupos',
				type: 'DELETE',
	...
	$("#botonGrupoBorrar").click(function(){
		...
		if (!text){
			...
			$.ajax({
				url: 'http://localhost:8888/api/grupos/'+text,
				type: 'DELETE',
				success: function(response) {
	...
	$("#botonGrupoInser").click(function(){
		...
		if (!text){
			...
			$.post("http://localhost:8888/api/grupos", text, function(){
				$("#textoGrupoInser").val('');
			}).done(function(){
